...
if (isset($_REQUEST['Alta'])) {
	extract($_REQUEST);
	$sql = "Insert into articulos_categorias ( categoria )
				values ('$categoria')";
...
if (isset($_REQUEST['Modificar'])) {
	extract($_REQUEST);
	$sql = "update articulos_categorias set 
			      categoria = '$categoria'
				  where id='$id'";
	$result = MySQLSESSION_ExecuteSQL($sql);
}
...
if ($action == 'modificar') {
	$sql = "select * from articulos_categorias where id=" . addslashes($id);
	$result = MySQLSESSION_ExecuteSQL($sql);
	...
	<div class="box span12">
		...
		<div class="box-content">
			<form class="form-horizontal" name="frm-usuarios" id="frm" method="post" enctype="multipart/form-data"
				action="categorias-editar.php?action=<?php echo $_REQUEST['action']; ?>">
				<fieldset>
					<div class="control-group">
						<label class="control-label" for="focusedInput">Categoria</label>
						<div class="controls">
							<input name="categoria" type="text" class="input-xlarge focused" id="focusedInput"
								value="<?php echo $row['categoria']; ?>" required="required">
					...
					<div class="form-actions">
						...
						<?php if ($_REQUEST['action'] == 'modificar') { ?>
							...
							<?php } ?>
						<input name="Volver" type="button" class="btn" value="Volver"
							onclick="location.href='categorias.php';" />
					</div>
				</fieldset>
			</form>
		</div>
